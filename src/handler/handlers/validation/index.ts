...
export class ValidationHandler {
  ...
  async #buildValidations() {
    ...
    for (const validationFilePath of validationFilePaths) {
      ...
      let importedFunction = (await import(validationFilePath)).default;
      ...
      if (typeof validationFunction !== 'function') {
        process.emitWarning(colors.yellow(`Ignoring: Validation file ${compactFilePath} does not export a function.`));
        continue;
      }
  ...
  async reloadValidations() {
    if (!this.#data.validationsPath) {
      throw new Error(
        colors.red('Cannot reload validations as "validationsPath" was not provided when instantiating Handler.'),
      );
    }
