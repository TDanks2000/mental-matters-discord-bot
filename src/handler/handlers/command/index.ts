...
import type { CommandHandlerData, CommandHandlerOptions, HandlerInteraction } from './types';
...
export class CommandHandler {
  ...
  async init() {
    ...
    if (devOnlyCommands.length && !this.#data.devGuildIds.length) {
      process.emitWarning(colors.yellow('You have commands marked as "devOnly", but "devGuildIds" have not been set.'));
    }

    if (devOnlyCommands.length && !this.#data.devUserIds.length && !this.#data.devRoleIds.length) {
      process.emitWarning(
        colors.yellow('You have commands marked as "devOnly", but "devUserIds" or "devRoleIds" have not been set.'),
      );
    }
  ...
  async #buildCommands() {
    ...
    for (const commandFilePath of commandFilePaths) {
      ...
      const importedObj = await import(commandFilePath);
      ...
      if (!commandObj.data) {
        process.emitWarning(colors.yellow(`Ignoring: Command file ${compactFilePath} does not export "data".`));
        continue;
      }

      if (!commandObj.data.name) {
        process.emitWarning(colors.yellow(`Ignoring: Command file ${compactFilePath} does not export "data.name".`));
        continue;
      }

      if (!commandObj.run) {
        process.emitWarning(colors.yellow(`Ignoring: Command file ${commandObj.data.name} does not export "run".`));
        continue;
      }

      if (typeof commandObj.run !== 'function') {
        process.emitWarning(
          colors.yellow(`Ignoring: Command file ${commandObj.data.name} does not export "run" as a function.`),
        );
        continue;
      }
  ...
  handleCommands() {
    ...
    this.#data.client.on('interactionCreate', async (interaction) => {
      ...
      const targetCommand = this.#data.commands.find((cmd) => cmd.data.name === interaction.commandName);
      ...
      const executor = async () => {
        ...
        if (this.#data.validationHandler) {
          ...
          for (const validationFunction of this.#data.validationHandler.validations) {
            const stopValidationLoop = await validationFunction({
              ...
              commandObj,
              client: this.#data.client,
              handler: this.#data.HandlerInstance,
            });
        ...
        // If custom validations pass and !skipBuiltInValidations, run built-in Handler validation functions
  ...
  async reloadCommands(type?: ReloadOptions) {
    if (!this.#data.commandsPath) {
      throw new Error(
        colors.red('Cannot reload commands as "commandsPath" was not provided when instantiating Handler.'),
      );
    }
