...
export default async function loadCommandsWithRest(props: LoadCommandsWithRestProps) {
  if (props.reloading) {
    if (props.client.isReady()) {
      await handleLoading(props.client, props.commands, props.devGuildIds, props.reloading, props.type);
    } else {
 ...
 */
async function loadGlobalCommands(client: Client<true>, commands: CommandFileObject[], reloading?: boolean) {
  const requestBody = commands.map((cmd) => cmd.data);

  await client.application.commands.set(requestBody as ApplicationCommandDataResolvable[]).catch((error) => {
    throw new Error(colors.red(`Error ${reloading ? 'reloading' : 'loading'} global application commands.\n`), error);
  });

  console.log(colors.green(`${reloading ? 'Reloaded' : 'Loaded'} ${requestBody.length} global commands.`));
}
...
async function loadDevCommands(
  ...
  for (const guildId of guildIds) {
    ...
    await targetGuild.commands.set(requestBody as ApplicationCommandDataResolvable[]).catch((error) => {
      throw new Error(
        colors.red(
          `Error ${reloading ? 'reloading' : 'loading'} developer application commands in guild "${
